...
module.exports.validateVendor = function (vendorData, callback) {
    ...
    // if (!vendorData.vendor_phone || !vendorData.vendor_phone.match(phoneRegex)) {
    //     return callback(null, { success: false, msg: "Please Enter a valid Phone Number." });
    // }
    ...
    bizzCat.findById({ '_id': objectid(vendorData.cat_id) }, function (err, catData) {
        if (err)
    ...
    bizzCat.findById({ '_id': objectid(vendorData.cat_id) }, function (err, catData) {
        ...
        if (catData) {
            ...
            if (vendorData.vendor_phone) {
                if (Array.isArray(vendorData.vendor_phone))
                    vendor_phone_data.vendor_phone = vendorData.vendor_phone;
                else
                    vendor_phone_data.push(vendorData.vendor_phone);
                for (var i = 1; i <= vendorData.vendor_phone.length; i++) {
                    vendor_phone_data.push({
                        phone: vendorData.vendor_phone[i-1],
                        isOwner: (vendorData.isOwner.includes(i.toString()))?true:false,
                        isManager: vendorData.isManager.includes(i.toString())?true:false,
                        isWhatsapp: vendorData.isWhatsapp.includes(i.toString())?true:false,
                        isCall: vendorData.isCall.includes(i.toString())?true:false,
                        isSMS: vendorData.isSMS.includes(i.toString())?true:false,
                    });
                }
            }
            ...
            function (err) {
                    var vendorObject = new Vendor({
                        ...
                        "billing_start_date": moment(Date.now()).format(),
                        "billing_end_date": moment(Date.now()).add(1, 'M').format(),
                    });
            ...
            if (vendorData.vendor_phone) {
                ...
                for (var i = 1; i <= vendorData.vendor_phone.length; i++) {
                    ...
                    vendor_phone_data.push({
                        ...
                        isCall: vendorData.isCall.includes(i.toString())?true:false,
                        isSMS: vendorData.isSMS.includes(i.toString())?true:false,
                        // isOwner: (Array.isArray(vendorData.isOwner))?((vendorData.isOwner[i-1]==i) ? true : false):((vendorData.isOwner==i)? true : false),
                        // isManager: (Array.isArray(vendorData.isManager))?((vendorData.isManager[i-1]==i) ? true : false):((vendorData.isManager==i)? true : false),
                        // isWhatsapp: (Array.isArray(vendorData.isWhatsapp))?((vendorData.isWhatsapp[i-1]==i) ? true : false):((vendorData.isWhatsapp==i)? true : false),
                        // isCall: (Array.isArray(vendorData.isCall))?((vendorData.isCall[i-1]==i) ? true : false):((vendorData.isCall==i)? true : false),
                        // isSMS: (Array.isArray(vendorData.isSMS))?((vendorData.isSMS[i-1]==i) ? true : false):((vendorData.isSMS==i)? true : false),
                    });
                }
            }
            // console.log(vendor_phone_data);
            // return;
            if (!vendorData.willDoLater) {
                for (var i = 0; i < week.length; i++) {
                    vendor_timings.push({
...
module.exports.makeImagePrimary = function (data, callback) {
    ...
    console.log(data.primary_image);
