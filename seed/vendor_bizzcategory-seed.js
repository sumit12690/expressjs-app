var bc = require('../models/models/vendor_bizzCat');
...
var done = 0;
for (var i = 0; i < vendor_bizzCat.length; i++) {
  vendor_bizzCat[i].save(function (err, result) {
    console.log(err);
    done++;
    if (done === vendor_bizzCat.length) {
      exit();
    }
  });
}
